...
class RoomProfileController(private val service: RoomProfileService) {
    ...
    @PostMapping
    fun add(@RequestBody roomProfile: RoomProfileRequest): ResponseEntity<RoomProfileDTO> {
        return ResponseEntity.status(HttpStatus.CREATED).body(service.add(roomProfile))
    }

    @PutMapping("/{id}")
    fun update(@RequestBody roomProfile: RoomProfileRequest, @PathVariable("id") id: Long): ResponseEntity<RoomProfileDTO> {
